    ...
    if request.method == 'POST':
        ...
        if action == 'update_info':
            ...
            if not bcrypt.checkpw(current.encode('utf-8'), stored.encode('utf-8')):
                ...
                # Check new password against current + last 3
                all_recent = [stored] + [row[0] for row in recent_hashes]
                reused = any(
                    bcrypt.checkpw(new_pass.encode('utf-8'), h.encode('utf-8'))
                    for h in all_recent
                )

                if reused:
                    flash("New password must be different from your last 3 passwords.", "error")
                else:
                    ...
                    # Keep only last 3 entries in history per user
                    cursor.execute("""
